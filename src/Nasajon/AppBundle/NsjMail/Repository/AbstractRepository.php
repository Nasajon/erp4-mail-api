...
use LogicException;
use Nasajon\AppBundle\NsjMail\Exceptions\RepositoryException;
use Nasajon\AppBundle\NsjMail\Request\Filter;
...
abstract class AbstractRepository {
    ...
    public function processApiReturn($retorno, $entity) {
        $resposta = json_decode($retorno, true);

        if ($resposta['codigo'] == "ERRO") {
            throw new RepositoryException($resposta['mensagem']);
        } elseif ($resposta['codigo'] == "CONFLITO") {
            throw new OptimisticLockException($resposta['mensagem'], $entity);
        }
